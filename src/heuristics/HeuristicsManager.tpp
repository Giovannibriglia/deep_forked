    ...
    switch (m_used_heuristics) {
        ...
        case Heuristics::GNN:
            ...
            ExitHandler::exit_with_message(
                ...
                "GNN heuristics selected, but neural network support (onnx handler) is not "
        ...
        case Heuristics::RL_H:
            ...
            if (Configuration::get_instance().get_search_strategy() != SearchType::RL) {
                ExitHandler::exit_with_message(
                    ExitHandler::ExitCode::HeuristicsBadDeclaration,
                    "RL heuristic selected, but wrong search_type selected. RL heuristics only works with RL search type.");
            }
            break;
#else
            ExitHandler::exit_with_message(
                ExitHandler::ExitCode::HeuristicsBadDeclaration,
                "RL heuristics selected, but neural network support (onnx handler) is not "
                "enabled or linked. Please recompile with the nn option.");
            break;
#endif
        default:
...
int HeuristicsManager<StateRepr>::get_heuristic_value(
            ...
            if (eState.get_heuristics_evaluation_times() == 0) {
                ...
                switch (Configuration::get_instance().get_RL_heuristics()) {
                    case RLHeuristicType::MIN: {
                        ret_heuristics = std::min(eState.get_heuristic_value(), static_cast<short>(eState.get_old_heuristic_value()));
                        break;
                    }
                    case RLHeuristicType::MAX: {
                        ret_heuristics = std::max(eState.get_heuristic_value(), static_cast<short>(eState.get_old_heuristic_value()));
                        break;
                    }
                    case RLHeuristicType::AVG: {
                        ret_heuristics = (eState.get_old_heuristic_value() * eState.get_heuristics_evaluation_times() +
                                    eState.get_heuristic_value()) / (eState.get_heuristics_evaluation_times() + 1);
                        break;
                    }
                    default:
                        ExitHandler::exit_with_message(
                            ExitHandler::ExitCode::HeuristicsBadDeclaration,
                            "Wrong RL Heuristic Selection in HeuristicsManager. Please check the "
                            "RL heuristic type.");
                        break;
                }
            }
    ...
    return
            -
            1; // Default return value, should not be reached.
}
