...
FringeTensor FringeEvalRL<StateRepr>::fringe_to_tensor_minimal(
  ...
  const std::vector<State<StateRepr>> cropped_states(
      states.begin(), states.begin() + dim_fringe);
...
std::vector<float> FringeEvalRL<StateRepr>::get_score(
  ...
  if (ArgumentParser::get_instance().get_dataset_separated()) {
    ...
    for (size_t i = 0; i < num_edges_goal; ++i) {
      edge_index_data_goal[i] =
          (goal_tensor.edge_src[i]); // First row: edge_src
    ...
    const std::array<int64_t, 2> edge_attr_shape_goal{
        static_cast<int64_t>(num_edges_goal), 1};
...
template <StateRepresentation StateRepr>
std::vector<float>
FringeEvalRL<StateRepr>::rankScores(const std::vector<float> &scores) const {
  int n = scores.size();
