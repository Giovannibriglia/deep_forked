...
void FringeEvalRL<StateRepr>::initialize_onnx_model() {
    ...
    try {
        ...
        ExitHandler::exit_with_message(ExitHandler::ExitCode::FringeModelLoadError,
...
FringeTensor FringeEvalRL<StateRepr>::fringe_to_tensor_minimal(
    ...
    switch (ArgumentParser::get_instance().get_dataset_type()) {
        ...
        case DatasetType::MAPPED:
            //Here pay attention to scale the id of the nodes by the number of previous inserted nodes
        default: {
            ExitHandler::exit_with_message(ExitHandler::ExitCode::FringeNotImplementedError,
    ...
    if (is_merged) {
        ExitHandler::exit_with_message(
            ...
            "Merged dataset type not supported for Fringe inference.");
    } else {
        ExitHandler::exit_with_message(
            ExitHandler::ExitCode::FringeNotImplementedError,
            "Separated dataset type not supported for Fringe inference.");
    }
    ...
    auto dim_fringe = std::min(MAX_ELEMENTS_IN_RL_FRINGE, states.size());
    const std::vector<State<StateRepr> > cropped_states(states.begin(), states.begin() + dim_fringe);

    for (auto state: cropped_states) {
        ...
        fringe_tensor_ret.real_node_ids.insert(fringe_tensor_ret.real_node_ids.end(), state_real_nodes_ids.begin(),
                                               state_real_nodes_ids.end());
        fringe_tensor_ret.membership.insert(fringe_tensor_ret.membership.end(), static_cast<size_t>(number_of_nodes),
                                            static_cast<int64_t>(state_number));

        const auto state_edges_src = state_tensor.edges_src;
        const auto state_edges_dst = state_tensor.edges_dst;
    ...
    // Here combine the various tensor in one, each state has it. Need to modify
    // the search so that it modies a list of states but double

    // The fringe will have n spots, the first m will be the newly expanded and
    // the remaining will be the top of the previous fringe. Everything will be
    // assigned a score, when something exceed the fringe will be
    //  assigned its score using a different heuristics? Or maybe using the depth?

    /*Facciamo finta di avere 4 stati e fringe massima di 5 stati, espandiamo il
     * piu' promettente che ne genera altri 4. Questo significa che teniamo i 4
     * stati piu' il secondo piu' promettente. Ora ripetiamo e di nuovo generiamo
     * altri 4 stati. Questo vuol dire che nella coda fuori dalla fringe abbiamo 7
     * stati che non sono comparabili tra loro, cosa facciamo? Come prediamo i
     * piu' promettenti tra quei 7 che non hanno mai fatto una frangia insime e
     * quindi non possono essere ordinati? Abbiamo ordinamento tra alcuni subset
     * ma non tutti. Forse ci conviene usare un'altra euristica e garantire
     * l'ordinamento dei subset che abbiamo controllato??*/
}
...
std::vector<float> FringeEvalRL<StateRepr>::get_score(const std::vector<State<StateRepr> > &states) {
    ...
    Ort::Value membership_tensor = Ort::Value::CreateTensor<uint64_t>(
    ...
    if (ArgumentParser::get_instance().get_dataset_separated()) {
        ...
        std::vector<int64_t> edge_index_data_goal(2 * num_edges_goal);
        for (size_t i = 0; i < num_edges_goal; ++i) {
            edge_index_data_goal[i] = (goal_tensor.edge_src[i]); // First row: edge_src
            edge_index_data_goal[num_edges_goal + i] =
                    (goal_tensor.edge_dst[i]); // Second row: edge_dst
        }

        const std::array<int64_t, 2> edge_index_shape_goal{
            2, static_cast<int64_t>(num_edges_goal)
        };
        Ort::Value edge_index_goal_tensor = Ort::Value::CreateTensor<int64_t>(
            memory_info, edge_index_data_goal.data(), edge_index_data_goal.size(),
            edge_index_shape_goal.data(), edge_index_shape_goal.size());

        // Construct edge_attr goal_tensor: shape [num_edges, 1]
        std::vector<float> edge_attrs_float_goal(goal_tensor.edge_attrs.begin(),
                                                 goal_tensor.edge_attrs.end());
        const std::array<int64_t, 2> edge_attr_shape_goal{
            static_cast<int64_t>(num_edges_goal),
            1
        };
        Ort::Value edge_attr_goal_tensor = Ort::Value::CreateTensor<float>(
            memory_info, edge_attrs_float_goal.data(), edge_attrs_float_goal.size(),
            edge_attr_shape_goal.data(), edge_attr_shape_goal.size());

        // Construct state_batch goal_tensor: shape [-1]
        std::vector<int64_t> state_batch_data_goal(num_nodes, 0);
        const std::array<int64_t, 1> state_batch_shape_goal{
            static_cast<int64_t>(state_batch_data_goal.size())
        };
        Ort::Value state_batch_goal_tensor = Ort::Value::CreateTensor<int64_t>(
            memory_info, state_batch_data_goal.data(), state_batch_data_goal.size(),
            state_batch_shape_goal.data(), state_batch_shape_goal.size());


        input_tensors.emplace_back(std::move(real_node_ids_goal_tensor));
        input_tensors.emplace_back(std::move(edge_index_goal_tensor));
        input_tensors.emplace_back(std::move(edge_attr_goal_tensor));
        input_tensors.emplace_back(std::move(state_batch_goal_tensor));
    }
    ...
    return rankScores(output_data);
}


template<StateRepresentation StateRepr>
std::vector<float> FringeEvalRL<StateRepr>::rankScores(const std::vector<float>& scores) const {
    int n = scores.size();

    // Pair each score with its original index
    std::vector<std::pair<float, int>> paired;
    for (int i = 0; i < n; ++i) {
        paired.push_back({scores[i], i});
    }
