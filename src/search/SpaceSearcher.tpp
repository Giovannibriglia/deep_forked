...
bool SpaceSearcher<StateRepr, Strategy>::search(
    ...
    if (ArgumentParser::get_instance().get_execute_plan()) {
        ...
        result = (num_threads <= 1)
                     ? search_sequential(thread_safe_initial,
...
bool SpaceSearcher<StateRepr, Strategy>::search_sequential(
    ...
    const bool is_RL_search = configuration.get_search_strategy() == SearchType::RL;
    ...
    while (!m_strategy.empty()) {
        ...
        if (is_RL_search && fringe_RL.size() >= static_cast<size_t>(RL_node_to_add)) {
